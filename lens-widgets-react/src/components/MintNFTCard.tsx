...
const MintNFTCard = ({ metadata } : { metadata?: MintableNFT }) => {
  if (!metadata) return null;

  const chainInfo = getZoraChainInfo(metadata.chainId);

  return (
    <div className="">
      <img className="h-[400px] w-full rounded-t-xl object-cover" src={metadata.image} />
      <div className="flex justify-between border-t border-b px-3 py-4 border-dark-grey">
